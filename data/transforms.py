...
def hflip(image, target, etarget):
    flipped_image = image
    if random.random() < 0.5:
        ...
        if etarget is not None and "masks" in etarget:
            etarget = etarget.copy()
            emask = etarget['masks']
            etarget['masks'] = np.flip(emask, (1))
    if random.random() < 0.5:
    ...
    if etarget is not None and "masks" in etarget:
        emask = etarget['masks']
        etarget['masks'] = np.rot90(emask, k=rotate_choice, axes=(1, 2))
        return flipped_image, target, etarget
...
def pad(image, target, etarget, region):
    z, x, y = image.shape
    nx, ny = region
    x_s = (x - nx) // 2
    y_s = (y - ny) // 2
    ...
    if etarget is not None and "masks" in etarget:
        emask = etarget["masks"]
        if x > nx and y > ny:
            emask_padded = emask[:, x_s:x_s + nx, y_s:y_s + ny]
        else:
            emask_padded = np.zeros((z, nx, ny), dtype=np.float32)
            if x <= nx and y > ny:
                emask_padded[:, x_c:x_c + x, :] = emask[:, :, y_s:y_s + ny]
            elif x > nx and y <= ny:
                emask_padded[:, :, y_c:y_c + y] = emask[:, x_s:x_s + nx, :]
            else:
                emask_padded[:, x_c:x_c + x, y_c:y_c + y] = emask[:, :, :]

        etarget['masks'] = emask_padded
        return slice_padded, target, etarget
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, target, etarget=None):
        if random.random() < self.p:
            return hflip(img, target, etarget)
...
class Rescale(object):
    ...
    def __call__(self, imgs, targets=None, etargets=None):
        img = imgs[0]
        ...
        if etargets is not None:
            etarget = etargets[0]
            scale_vector_etarget = etargets[1]
        ...
        if "masks" in target:
            ...
            target['masks'] = mask
        if etargets is not None and "masks" in etarget:
            emask = etarget['masks']
            emask = transform.rescale(emask[0, :, :],
                                     scale_vector_target,
                                     order=0,
                                     preserve_range=True,
                                     anti_aliasing=False,
...
class PadOrCropToSize(object):
    ...
    def __call__(self, img, target, etarget=None):
        crop_height, crop_width = self.size
        if etarget is not None:
            padded_img, padded_target, padded_etarget = pad(img, target, etarget, (crop_height, crop_width))
            return padded_img, padded_target, padded_etarget
        else:
            padded_img, padded_target = pad(img, target, etarget, (crop_height, crop_width))
...
class RandomRotate(object):
    ...
    def __call__(self, img, target, etarget=None):
        ...
        if etarget is not None:
            emask = etarget['masks']
            emask = (emask.copy()).astype(np.int32)
            rotated_emask = F.rotate(torch.from_numpy(emask), angle, PIL.Image.NEAREST, self.expand, self.center)
            rotated_emask = rotated_emask.numpy()
            etarget["masks"] = rotated_emask
            return rotated_img, target, etarget
        return rotated_img, target


class RandomRotate_90(object):
    def __init__(self):
        pass
    ...
    def __call__(self, img, target):
        ...
        img = img.copy()
        rotated_img = F.rotate(torch.from_numpy(img), angle, PIL.Image.NEAREST)
        rotated_img = rotated_img.numpy()
        #  if "masks" in target:
        mask = target['masks']
        mask = (mask.copy()).astype(np.int32)
        rotated_mask = F.rotate(torch.from_numpy(mask), angle, PIL.Image.NEAREST)
        rotated_mask = rotated_mask.numpy()
        target["masks"] = rotated_mask
        return rotated_img, target
        ...
        for k, v in target.items():
            if not isinstance(v, str):
                if torch.is_tensor(v) or isinstance(v, (list, tuple)):
                    ...
                    v = v.copy().astype(np.int32)
                    target[k] = torch.tensor(v).type(torch.LongTensor)
        if not torch.is_tensor(img):
            img = img.copy()
            img = torch.from_numpy(img)
        if etarget is not None:
            for k, v in etarget.items():
                if not isinstance(v, str):
                    if torch.is_tensor(v) or isinstance(v, (list, tuple)):
                        if torch.is_tensor(v):
                            pass
                        else:
                            etarget[k] = torch.tensor(v).type(torch.LongTensor)
                    else:
                        v = v.copy().astype(np.int32)
                        etarget[k] = torch.tensor(v).type(torch.LongTensor)
            return img, target, etarget
...
class Normalize(object):
    ...
    def __call__(self, image, target=None, etarget=None):
...
class Compose(object):
    ...
    def __call__(self, image, target, etarget=None):
        if etarget is not None:
            for t in self.transforms:
                image, target, etarget = t(image, target, etarget)
            return image, target, etarget
        for t in self.transforms:
